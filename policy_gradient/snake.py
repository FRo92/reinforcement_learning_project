...
# those dictionaries act a translation tools.
...
# this function takes in a 3 channel rgb image and converts it to grayscale
def rgb2gray(rgb):
    ...
    def is_alive(self):
        ...
        if len(self.body) > 1:
            if np.any(np.all(self.body[-1] == np.array(self.body[:-1]), axis=1)):
                self.alive = False
        if np.any((np.array(self.body) < 0) | (np.array(self.body) >= self.board_w), axis=None):
            self.alive = False
        return self.alive
    ...
    def obstacle_up(self):
        up = (np.any(np.all(self.snake.body[-1] + up_vect == np.array(self.snake.body[:]), axis=1)) \
              or np.any(0 > (self.snake.body[-1] + up_vect)) or np.any(
                    (self.snake.body[-1] + up_vect >= self.board_w)))
        return int(up)
    def obstacle_down(self):
        down = (np.any(np.all(self.snake.body[-1] + down_vect == np.array(self.snake.body[:]), axis=1)) \
                or np.any(0 > (self.snake.body[-1] + down_vect)) or np.any(
                    (self.snake.body[-1] + down_vect >= self.board_w)))
        return int(down)
    def obstacle_left(self):
        left = (np.any(np.all(self.snake.body[-1] + left_vect == np.array(self.snake.body[:]), axis=1)) \
                or np.any(0 > (self.snake.body[-1] + left_vect)) or np.any(
                    (self.snake.body[-1] + left_vect >= self.board_w)))
        return int(left)
    def obstacle_right(self):
        right = (np.any(np.all(self.snake.body[-1] + right_vect == np.array(self.snake.body[:]), axis=1)) \
                 or np.any(0 > (self.snake.body[-1] + right_vect)) or np.any(
                    (self.snake.body[-1] + right_vect >= self.board_w)))
        return int(right)
    def get_state(self):
        if self.pixels == True:
            if self.rgb_to_gray:
...
class VideoRecorder(object):
    ...
    def save_video(self, dir_name, game_counter, highscore):
        ...
        ani = animation.FuncAnimation(fig, update, frames=n_frames, blit=False, interval=150)
        ani.save(f"Gifs/{dir_name}/episode_{game_counter}.gif")
        plt.close(fig)
...
class Board(Statespace_manager, Reward_manager, Actionspace_manager,):
    def reset_board(self):
        self.snake = Snake(self.board_w, self.board_h)
        self.apple_coords = np.random.randint(0, self.board_w, 2)
        self.score = 0

    def __init__(self, board_w, board_h,dir_name,pixels=True,rgb_to_gray = False,n_stacked_frames = 1, skip=1000):
        ...
        self.board = np.zeros((board_w, board_h, 3))
        self.snake = Snake(board_w, board_h)
        self.apple_coords = np.random.randint(0, board_w, 2)
    ...
    def play(self, direction):
        ...
        if not self.snake.is_alive():
            ...
            # self.scores.append(self.score)
            if board.game_counter % 1000 == 0 or self.score == self.highscore:
...
if __name__ == "__main__":
    ...
    board = Board(20, 30, skip=9_000_000, pixels = False, rgb_to_gray=True, dir_name = "PolicyGradient",  n_stacked_frames = 2)

    agentPolicy = PolicyGradient.model(0.99)
    #agentPolicyCNN = PolicyGradientCNN.model(0.99)
    state = board.get_state()

    for s in range(100_000_000):
        '''agregamos un contador de tiempo'''
        now = time.time()
        if now - start > 3000 * 60:
            break
        else:
            done = False
            total_reward = 0
